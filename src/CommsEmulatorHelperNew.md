                            ...
                            const gz::sim::EntityComponentManager &_ecm) 
{
  ...
  for (const auto &robot1Entry : this->dataPtr->robotPositions) 
  {
    ...
    for (const auto &robot2Entry : this->dataPtr->robotPositions) 
    {
      ...
      double Bandwidth = 0; //Mbps
      double previousBandwidth = 0;
      double HYSTERESIS = 5; // hysterisis
      if (SNR > 25)
      {
        Bandwidth = 72; //Mbps
      }
      ...
      // // Hysteresis
      // Use the previousBandwidth to decide whether to stick or change state
      // if (previousBandwidth == 72.0) {
      //     // Already in the highest state; only drop if SNR falls well below 25
      //     if (SNR >= 25 - HYSTERESIS) {
      //         Bandwidth = 72.0;
      //     } else if (SNR >= 11 + HYSTERESIS) {
      //         Bandwidth = 28.9;
      //     } else if (SNR >= 2 + HYSTERESIS) {
      //         Bandwidth = 7.2;
      //     } else {
      //         Bandwidth = 0.0; // or an error/default case
      //     }
      // }
      // else if (previousBandwidth == 28.9) {
      //     // In the medium state; check for switching up or down
      //     if (SNR >= 25 + HYSTERESIS) {  // SNR high enough to jump to 72 Mbps
      //         Bandwidth = 72.0;
      //     } else if (SNR >= 11 - HYSTERESIS) { // Remain in 28.9 Mbps
      //         Bandwidth = 28.9;
      //     } else if (SNR >= 2 + HYSTERESIS) {  // Drop to 7.2 Mbps
      //         Bandwidth = 7.2;
      //     } else {
      //         Bandwidth = 0.0;
      //     }
      // }
      // else if (previousBandwidth == 7.2) {
      //     // In the low state; only move up if SNR is sufficiently higher
      //     if (SNR >= 25 + HYSTERESIS) {
      //         Bandwidth = 72.0;
      //     } else if (SNR >= 11 + HYSTERESIS) {
      //         Bandwidth = 28.9;
      //     } else if (SNR >= 2 - HYSTERESIS) {  // Note: 2 - 5 gives -3, so if SNR is at least -3, remain here
      //         Bandwidth = 7.2;
      //     } else {
      //         Bandwidth = 0.0;
      //     }
      // }
      // else {
      //     // Initial assignment (or if previousBandwidth isn't set)
      //     if (SNR >= 25 + HYSTERESIS) {
      //         Bandwidth = 72.0;
      //     } else if (SNR >= 11 + HYSTERESIS) {
      //         Bandwidth = 28.9;
      ...
      // // Save the new bandwidth for hysteresis in future calls
